...
JNB_VENV=""                 # Path to virtual environment
...
case $JNB_SOFTWARE_STACK in
        ...
        new)
        if [ "$JNB_NUM_GPU" -gt "0" ]; then
            JNB_MODULE_COMMAND="gcc/8.2.0 python/3.8.5 eth_proxy $JNB_JULIA"
            JNB_MODULE_COMMAND+=" ${JNB_EXTRA_MODULES[@]}"
            echo -e "Using new software stack (gcc/8.2.0 python/3.8.5 eth_proxy $JNB_JULIA ${JNB_EXTRA_MODULES[@]})"
        else
            JNB_MODULE_COMMAND="gcc/8.2.0 python/3.8.5 eth_proxy $JNB_JULIA"
            JNB_MODULE_COMMAND+=" ${JNB_EXTRA_MODULES[@]}"
            echo -e "Using new software stack (gcc/8.2.0 python/3.8.5 eth_proxy $JNB_JULIA ${JNB_EXTRA_MODULES[@]})"
